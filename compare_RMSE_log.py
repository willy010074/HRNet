...
wb = xlwt.Workbook()
wb_sheet = wb.add_sheet('RMSE_log')
...
for name in image_list:
    ...
    for i in range(test.shape[0]):
        for j in range(test.shape[1]):
            a = int(test[i,j])
            b = int(gt[i,j])
            total = total + (math.log10(a) - math.log10(b))*(math.log10(a) - math.log10(b))
    ...
    index += 1
#    print("RMSE_log : ", total)
#    print("----------------------------------------------------------------------")
#
#print("mean RMSE_log : ", mean_rmse/2270)

wb.save('result_compare/RMSE_log.xls')
